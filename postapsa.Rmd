...
knitr::opts_knit$set(root.dir = "/Users/hyoon/Desktop/apsa_interdistrict")
```
...
Disaster data (based on codes.R)
```{r}
# fema: county-level
...
Joining disaster to district level
```{r}
...
try <- unique(trial[c("year.x", "cong","close", "pri", "Total Deaths", "Total Affected", "Total Damages, Adjusted ('000 US$)")]) %>% 
  mutate_at(c("Total Deaths", "Total Affected", "Total Damages, Adjusted ('000 US$)"), funs(as.numeric))


## coding decision time
# there are incidents where there were multiple disasters in one district
# should i incorporate disasters that happened far before the election season?

# show ones with more than one disasters in the district
try %>% group_by(year.x, cong) %>% summarize(num = n()) 

# check for duplicates
# these only include districts that had multiple disasters in one election year
...
dupe <- try_dupe %>% group_by(year.x, cong) %>% summarize(deaths = sum(`Total Deaths`),
...
try_dupe0 <- try %>% group_by(year.x, cong) %>% filter(n()==1) %>% dplyr::rename(deaths = 5,
                                                                          affected = 6,
                                                                          damages = 7) %>% mutate(num = 1) 

try <- rbind(try_dupe0, try_dupe2)


## another dataset with disasters including <300 (try2)
dupe3 <- try_dupe %>% group_by(year.x, cong) %>% filter(close <= 300) %>% summarize(deaths = sum(`Total Deaths`),
                                       affected = sum(`Total Affected`),
                                       damages = sum(`Total Damages, Adjusted ('000 US$)`),
                                       num = n()) 
...
# try includes all disasters, try2 only includes ones happened 300 days before election day
```

Joining disaster and dime
```{r}
...
agg$deaths <- ifelse(is.na(agg$deaths)==T, 0, agg$deaths)
agg$affected <- ifelse(is.na(agg$affected)==T, 0, agg$affected)
agg$damages <- ifelse(is.na(agg$damages)==T, 0, agg$damages)
agg$num <- ifelse(is.na(agg$num)==T, 0, agg$num)


# join with try
agg1 <- left_join(agg, try,
                 by = c("cycle" = "year.x",
                        "district" = "cong"))

agg1[duplicated(agg1[,1:3]),] # check for duplicates

agg1 <- agg1 %>% ungroup(.) %>% mutate(num = ifelse(is.na(.$num)==T, 0, .$num),
                      deaths = ifelse(is.na(.$deaths)==T, 0, .$deaths),
                      affected = ifelse(is.na(.$affected)==T, 0, .$affected),
                      damages = ifelse(is.na(.$damages)==T, 0, .$damages))

# join with try2
agg2 <- left_join(agg, try2,
                  by = c("cycle" = "year.x",
                        "district" = "cong"))

agg2[duplicated(agg2[,1:3]),] # check for duplicates

agg2 <- agg2 %>% ungroup(.) %>% mutate(num = ifelse(is.na(.$num)==T, 0, .$num),
                      deaths = ifelse(is.na(.$deaths)==T, 0, .$deaths),
                      affected = ifelse(is.na(.$affected)==T, 0, .$affected),
                      damages = ifelse(is.na(.$damages)==T, 0, .$damages))
```
...
ggplot(agg %>% filter(party %in% c(100, 200)) %>% filter(cycle == 2014),
       aes(x=pct.out)) + 
  geom_histogram(aes(color = party), alpha = 0.4) +
  scale_color_manual(values=c( "#56B4E9", "#E69F00"), name = "Party", labels = c("Dem", "Rep")) 
```

Coding disasters
```{r}
# want to make a function where i can put in IV and threshold

quantile(agg$deaths[agg$num > 0], probs=seq(0,1,0.1)) # >68
...
# it seems like agg and agg1 is the same thing

# agg1 includes all disasters
quantile(agg1$deaths[agg1$num > 0], probs=seq(0,1,0.1)) # >68
quantile(agg1$affected[agg1$num > 0], probs=seq(0,1,0.1)) # >225000
quantile(agg1$damages[agg1$num > 0], probs=seq(0,1,0.1)) # >9916769

agg1 <- agg1 %>% mutate(nd_death = ifelse(deaths >= 68, 1, 0),
                      nd_affected = ifelse(affected >=225000, 1, 0),
                      nd_damages = ifelse(damages >= 9916769, 1, 0))

# agg2 includes <300
quantile(agg2$deaths[agg2$num > 0], probs=seq(0,1,0.1)) # >46
quantile(agg2$affected[agg2$num > 0], probs=seq(0,1,0.1)) # >225000
quantile(agg2$damages[agg2$num > 0], probs=seq(0,1,0.1)) # >9916769

agg2 <- agg2 %>% mutate(nd_death = ifelse(deaths >= 46, 1, 0),
                      nd_affected = ifelse(affected >=225000, 1, 0),
                      nd_damages = ifelse(damages >= 9916769, 1, 0))
```
...
# all cases
feols(n.in ~ nd_death | district + cycle + party, agg1)
feols(n.out ~ nd_death | district + cycle + party, agg1)
feols(amt.in ~ nd_death | district + cycle + party, agg1)
feols(amt.out ~ nd_death | district + cycle + party, agg1)

# all cases - interactive FE
feols(n.in ~ nd_death | district^party + cycle^party, agg1)
feols(n.out ~ nd_death | district^party + cycle^party, agg1)
feols(amt.in ~ nd_death | district^party + cycle^party, agg1)
feols(amt.out ~ nd_death | district^party + cycle^party, agg1)
...
# <300 - interactive FE
feols(n.in ~ nd_death | district^party + cycle^party, agg2)
feols(n.out ~ nd_death | district^party + cycle^party, agg2)
feols(amt.in ~ nd_death | district^party + cycle^party, agg2)
feols(amt.out ~ nd_death | district^party + cycle^party, agg2)

feols(n.in ~ nd_damages | district + cycle + party, agg1)
feols(n.out ~ nd_damages | district + cycle + party, agg1)
feols(amt.in ~ nd_damages | district + cycle + party, agg1)
feols(amt.out ~ nd_damages | district + cycle + party, agg1)

feols(n.in ~ nd_damages | district^party + cycle^party, agg1)
feols(n.out ~ nd_damages | district^party + cycle^party, agg1)
feols(amt.in ~ nd_damages | district^party + cycle^party, agg1)
feols(amt.out ~ nd_damages | district^party + cycle^party, agg1)
...
feols(n.in ~ nd_damages | district^party + cycle^party, agg2)
feols(n.out ~ nd_damages | district^party + cycle^party, agg2)
feols(amt.in ~ nd_damages | district^party + cycle^party, agg2)
feols(amt.out ~ nd_damages | district^party + cycle^party, agg2)


### effect of nd on share

feols(pct.out ~ nd_death | district + cycle + party, data=agg)
feols(pct.amt.out ~ nd_death | district + cycle + party, data=agg)

feols(pct.out ~ nd_death | district^party + cycle^party, data=agg)
feols(pct.amt.out ~ nd_death | district^party + cycle^party, data=agg)

feols(pct.out ~ nd_affected | district + cycle + party, data=agg)
feols(pct.amt.out ~ nd_affected | district + cycle + party, data=agg)

feols(pct.out ~ nd_affected | district^party + cycle^party, data=agg)
feols(pct.amt.out ~ nd_affected | district^party + cycle^party, data=agg)

feols(pct.out ~ nd_damages | district + cycle + party, data=agg)
feols(pct.amt.out ~ nd_damages | district + cycle + party, data=agg)

feols(pct.out ~ nd_damages | district^party + cycle^party, data=agg)
feols(pct.amt.out ~ nd_damages | district^party + cycle^party, data=agg)
...
agg <- agg %>% ungroup(.) %>% mutate(nd_death2 = ifelse(.$close < 300 & .$nd_death == 1, 1, 0),
                                     nd_affect2 = ifelse(.$close < 300 & .$nd_affected == 1, 1, 0),
                                     nd_both2 = ifelse(.$close < 300 & .$nd_damages == 1, 1, 0))
# seems like this is just making agg2 within agg

feols(pct.out ~ nd_death2 | district + cycle + party, data=agg)
feols(pct.amt.out ~ nd_death2 | district + cycle + party, data=agg)

feols(pct.out ~ nd_death2 | district^party + cycle^party, data=agg)
feols(pct.amt.out ~ nd_death2 | district^party + cycle^party, data=agg)

feols(pct.out ~ nd_affect2 | district + cycle + party, data=agg)
feols(pct.amt.out ~ nd_affect2 | district + cycle + party, data=agg)

feols(pct.out ~ nd_affect2 | district^party + cycle^party, data=agg)
feols(pct.amt.out ~ nd_affect2 | district^party + cycle^party, data=agg)

feols(pct.out ~ nd_both2 | district + cycle + party, data=agg)
feols(pct.amt.out ~ nd_both2 | district + cycle + party, data=agg)

feols(pct.out ~ nd_both2 | district^party + cycle^party, data=agg) # but actually damages
feols(pct.amt.out ~ nd_both2 | district^party + cycle^party, data=agg)
...
feols(ab.dw1 ~ nd_death | district^party + cycle^party, data=winner)
feols(ab.dist.dw1 ~ nd_death | district^party + cycle^party, data=winner)
feols(ab.dw1 ~ nd_affect | district^party + cycle^party, data=winner)
feols(ab.dist.dw1 ~ nd_affect | district^party + cycle^party, data=winner)
feols(ab.dw1 ~ nd_both | district^party + cycle^party, data=winner)
feols(ab.dist.dw1 ~ nd_both | district^party + cycle^party, data=winner)
...
feols(ab.dw1 ~ nd_death2 | district^party + cycle^party, data=winner)
feols(ab.dist.dw1 ~ nd_death2 | district^party + cycle^party, data=winner)
feols(ab.dw1 ~ nd_affect2 | district^party + cycle^party, data=winner)
feols(ab.dist.dw1 ~ nd_affect2 | district^party + cycle^party, data=winner)
feols(ab.dw1 ~ nd_both2 | district^party + cycle^party, data=winner)
feols(ab.dist.dw1 ~ nd_both2 | district^party + cycle^party, data=winner)
...
feols(prob_dw1 ~ nd_death | cong^party + year^party, data=prob_anl)
feols(prob_dw2 ~ nd_death | cong^party + year^party, data=prob_anl)
feols(prob_dw1 ~ nd_affect | cong^party + year^party, data=prob_anl)
feols(prob_dw2 ~ nd_affect | cong^party + year^party, data=prob_anl)
feols(prob_dw1 ~ nd_both | cong^party + year^party, data=prob_anl)
feols(prob_dw2 ~ nd_both | cong^party + year^party, data=prob_anl)
...
feols(prob_dw1 ~ nd_death2 | cong^party + year^party, data=prob_anl)
feols(prob_dw2 ~ nd_death2 | cong^party + year^party, data=prob_anl)
feols(prob_dw1 ~ nd_affect2 | cong^party + year^party, data=prob_anl)
feols(prob_dw2 ~ nd_affect2 | cong^party + year^party, data=prob_anl)
feols(prob_dw1 ~ nd_both2 | cong^party + year^party, data=prob_anl)
feols(prob_dw2 ~ nd_both2 | cong^party + year^party, data=prob_anl)
